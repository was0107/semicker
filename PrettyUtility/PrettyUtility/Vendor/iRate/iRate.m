...
NSUInteger const iAppStoreID = 595252508;

NSUInteger const kUsesUntilPrompt = 5;  //使用次数
CGFloat const kDaysUntilPrompt = 5.0f;  //使用天数
CGFloat const kRemindPeriod = 5.0f;     //提示间隔
NSUInteger const kEventsUntilPrompt = 10;  
...
- (iRate *)init
{
    if ((self = [super init]))
    {
        ...
        self.usesUntilPrompt = kUsesUntilPrompt;    //使用次数
        self.eventsUntilPrompt = kEventsUntilPrompt; 
        self.daysUntilPrompt = kDaysUntilPrompt;    //使用天数
        self.remindPeriod = kRemindPeriod;          //提示间隔
        ...
        self.cancelButtonLabel = @"不，谢谢";//[self localizedStringForKey:@"No, Thanks"];
        //self.remindButtonLabel = @"以后提醒我";//[self localizedStringForKey:@"Remind Me Later"];
        self.rateButtonLabel = @"去评价";//[self localizedStringForKey:@"Rate It Now"];
...
- (NSURL *)ratingsURL
{
    ...
    NSUInteger appID = self.appStoreID;
    if (!appID) {
        appID = iAppStoreID;
    }
#ifdef __IPHONE_OS_VERSION_MAX_ALLOWED
    ...
    {
        return [NSURL URLWithString:[NSString stringWithFormat:iRateiOS6AppStoreURLFormat, (unsigned int)appID]];
    }
    else
    {
        return [NSURL URLWithString:[NSString stringWithFormat:iRateiOSAppStoreURLFormat, (unsigned int)appID]];
    }
    
#else
    
    return [NSURL URLWithString:[NSString stringWithFormat:iRateMacAppStoreURLFormat, (unsigned int)appID]];
    ...
    @synchronized (self)
    {
        @autoreleasepool
        {
            ...
            if (data)
            {
                ...
                //get url
                NSString *trackViewUrl = [self valueForKey:@"trackViewUrl" inJSON:json];
                self.appStoreURL = [NSURL URLWithString:trackViewUrl];
                ...
                if ((bundleID && [bundleID isEqualToString:self.applicationBundleID]) || self.debug)
                {
                    ...
                    if ([latestVersion compare:self.applicationVersion options:NSNumericSearch] == NSOrderedDescending)
                    {
                        ...
                        {
                            [self performSelectorOnMainThread:@selector(checkVersionUpdated:) withObject:latestVersion waitUntilDone:YES];
...
- (void)checkVersionUpdated:(NSString *)version
{
    NSString *message = [NSString stringWithFormat:@"检测到新版本v%@，您要去AppStore更新吗？",version];
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"版本更新"
                                                    message:message
                                                   delegate:self
                                          cancelButtonTitle:@"以后提示"
