...
@property (nonatomic, assign) CGFloat reverseTranslation;
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    view.superview.center = CGPointMake(self.bounds.size.width/2 + contentOffset.width,
										self.bounds.size.height/2 + contentOffset.height);
    ...
    [view.superview setFrameOrigin:NSMakePoint(self.bounds.size.width/2 + contentOffset.width,
											   self.bounds.size.height/2 + contentOffset.height)];
...
- (void)didTap:(UITapGestureRecognizer *)tapGesture
{
    ...
    if ([delegate respondsToSelector:@selector(carousel:didSelectItemAtIndex:)])
    {
        ...
        {
            if ([delegate respondsToSelector:@selector(carousel:didEndSelectItemAtIndex:)])
                [delegate carousel:self didEndSelectItemAtIndex:index];
        }
    }
}
    ...
    if (scrollEnabled)
    {
        switch (panGesture.state)
        {
            case UIGestureRecognizerStateBegan:
            {
                ...
                reverseTranslation  = !vertical? [panGesture translationInView:self].y: [panGesture translationInView:self].x;
                position = previousTranslation;
            ...
            case UIGestureRecognizerStateCancelled:
            {
				...
				if (!decelerating && (scrollToItemBoundary || (scrollOffset - [self clampedOffset:scrollOffset]) != 0.0f))
				{
                    ...
                    {
                        NSInteger directionTmp = (int)(startVelocity / fabsf(startVelocity));
                        [self scrollToItemAtIndex:self.currentItemIndex + directionTmp animated:YES];
                    }
                    else
                    {
                        [self scrollToItemAtIndex:self.currentItemIndex animated:YES];
