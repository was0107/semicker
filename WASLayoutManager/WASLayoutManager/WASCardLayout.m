...
{
    return [NSString stringWithFormat:@"%@ [hgap = %d, vgap = %d ] ",NSStringFromClass([self class]), self.hgap, self.vgap];
}
...
- (void) addLayoutComponent:(NSString *) name with:(Component *)comp
{
    ...
    [_contents addObject:[[[WASCard alloc] initWithName:name with:comp] autorelease]];
}
...
{
    NSUInteger ncomponents = [comp countComponents];
    int w = 0;
    int h = 0;
    for (int i = 0 ; i < ncomponents ; i++) {
