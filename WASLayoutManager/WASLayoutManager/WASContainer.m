...
//        _flowLayout.newAlign = WASFlowLayout.LEFT;
...
{
    if (index > (int)[_component count] || (index < 0 && index != -1)) {
        [NSException raise:kErrorDomain format:@"illegal component position11"];
    }
...
- (void) removeComponentAt:(int) index
{
    if (index < 0  || index >= (int)[_component count]) {
        return;
        [NSException raise:kErrorDomain format:@"illegal component position22"];
    }
...
- (void) removeComponent:(Component *)comp
{
    if (comp.superview == self) {
        int index =(int) [_component indexOfObject:comp];
        if (index >= 0 && index != NSUIntegerMax) {
